...
stock_names = [
    ...
    'ISGYO.IS', 'ISMEN.IS', 'KARSN.IS', 'KAYSE.IS', 'KCAER.IS', 
...
for ticker in stock_names:
    ...
    combined_df['Date 2'] = pd.to_datetime(combined_df['Date 2'])
    today = datetime.date.today()
    combined_df['ticker'] = ticker
    combined_df['days diff'] = (today - combined_df['Date 2'].dt.date).apply(lambda x: x.days)
    filtered_df = combined_df[combined_df['days diff'] < 6]
